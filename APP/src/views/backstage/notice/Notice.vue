<template>
    <div>
        ...
        <el-card class="noticePannel">
            ...
            <div class="add">
                <el-button type="primary" @click="addDialog = true" round>
                    <el-icon>
                        <Plus />
                    </el-icon>
                    <span>新增</span>
        ...
        <el-card class="noticeList">
            <el-table :data="tableData" style="width: 100%" ref="multipleTableRef"
                ...
                <el-table-column label="操作">
                    <template #default="scope">
                        <el-button type="primary" round plain :icon="Edit"
                            @click="showEditNoticeDialog(scope.row)">编辑</el-button>
        ...
        <el-dialog v-model="addDialog" title="新增通知" width="500" :before-close="handleClose">
            <el-form ref="addNoticeFormRef" :model="noticeForm" label-width="auto" style="max-width: 600px"
                :rules="noticeRules" status-icon>
            ...
            <template #footer>
                <div class="dialog-footer">
                    ...
                    <el-button type="primary" @click="noticeAdd(addNoticeFormRef)">
                        添加
                    </el-button>
                </div>
            </template>
        ...
        <el-dialog v-model="editDialog" title="修改通知" width="500">
            <el-form ref="editNoticeFormRef" :model="editNoticeForm" label-width="auto" style="max-width: 600px"
                :rules="noticeRules" status-icon>
                <el-form-item label="创建人" prop="creator">
            ...
            <template #footer>
                <div class="dialog-footer">
                    <el-button @click="addDialog = false">取消</el-button>
                    <el-button type="primary" @click="noticeUpdate(editNoticeFormRef)">
                        修改
                    </el-button>
                </div>
            </template>
...
const multipleDelete = async () => {
    axios.delete('/adminapi/notices/deletenotices', {
        ...
        ElMessage({
            message: res.data.success ? '批量删除成功' : res.data.message,
        ...
        if (valid) {
            ...
            axios.post('/adminapi/notices/addnotice', noticeForm).then(res => {
                if (res.data.success) {
                    ...
                    for(var i in noticeForm){
                        noticeForm[i] = ''
                    }
                    noticeForm.creator = store.userInfo.userid
                    currentPage.value = 1
                    getNoticeList(1)
                } else {
...
const showEditNoticeDialog = (form) => {
    editDialog.value = true
    for (var i in form) {
        editNoticeForm[i] = form[i]
    }
}
const noticeUpdate = async (form) => {
    await form.validate(async (valid) => {
        if (valid) {
            ...
            ElNotification({
                title: res.data.success ? '修改成功！' : '修改失败！',
                message: res.data.success ? '公告通知修改成功' : res.data.message,
                type: res.data.success ? 'success' : 'error'
            })
