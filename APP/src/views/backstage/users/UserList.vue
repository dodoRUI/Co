        ...
        <el-card class="userSearch">
            ...
            <el-select v-model="filterUserForm.gender" placeholder="性别" size="middle" style="width: 80px;">
                <el-option v-for="item in genderOptions" :key="item.value" :label="item.label" :value="item.value">
                    <el-icon v-if="item.value == 1">
                        <Male />
                    </el-icon>
                    <el-icon v-else-if="item.value == 0">
                        <Female />
                    </el-icon>
                    <el-icon v-else>
                        <CircleClose />
...
const userEdit = async (user) => {
    ...
    const res = await axios.post(`/adminapi/users/userlist`, { userid: user.userid })
    userForm.value = res.data.data[0]
}
// 确认用户修改提交表单
const userEditConfirm = async (userForm) => {
    const res = await axios.put(`/adminapi/users/userlist/${userForm.userid}`, userForm)
    if (!res.data.success) {
...
const userDelete = async (user) => {
    const res = await axios.delete(`/adminapi/users/userlist/deleteuser`,{params:{userid:user.userid,avatar:user.avatar}})
    if(res.data.success){
        getUserList()
...
const userFilter = async (userForm, page, size) => {
    ...
    const res = await axios.get('/adminapi/users/userlist', { params: { obj, page, size } })
    if (res.data.success) {
        tableData.value = res.data.data
        total.value = res.data.total
    } else {
...
const multipleDelete = async () => {
    const res = await axios.delete('/adminapi/users/userlist', {
        params: { users: multipleSelection.value }
    })
    ElMessage({
        message: res.data.success ? '批量删除成功' : '删除失败',
