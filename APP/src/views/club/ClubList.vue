<template>
    <div>
        ...
        <el-card class="filterPannel">
            <div class="search">
                <el-button type="primary" @click="addDialog = true" :icon="Plus">新增</el-button>
        ...
        <el-dialog v-model="addDialog" title="成立社团" width="500" :before-close="handleClose">
            ...
            <template #footer>
                <div class="dialog-footer">
                    ...
                    <el-button type="primary" round @click="clubAdd(clubForm)">
                        添加
                    </el-button>
                </div>
            </template>
...
const clubAdd = (clubForm) => {
    if (clubForm.club_name && clubForm.club_belong && clubForm.club_minister) {
...
const clubDelete = (club) => {
    const { username, club_name, club_minister, club_id } = club
    axios.delete('/adminapi/clubs/deleteclub', { params: { club_id, club_minister } }).then(res => {
        if (res.data.ActionType == 'OK') {
            ...
            ElNotification.error({
                title: `删除失败`,
                dangerouslyUseHTMLString: true,
                message: `<strong style="color:red">服务器出错，社团删除失败！</strong>`,
            })
        }
    })
}
