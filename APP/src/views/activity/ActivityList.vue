        ...
        <el-card class="actvtList">
            <el-table :data="tableData" style="width: 100%" ref="multipleTableRef" stripe>
                ...
                <el-table-column prop="club_name" label="社团名称" width="200">
                    <template #default="scope">
                        <el-button size="small" round @click="gotoClub(scope.row.club_id)">{{ scope.row.club_name }}</el-button>
                    </template>
                </el-table-column>
...
const getActvtList = async (page, size) => {
    ...
    let res = await axios.get(`/adminapi/activities/list`, { params: { club_id, page, size } })
...
const actvtFilter = async (form, page, size) => {
    ...
    const club_id = store.userInfo.role == 1 ? store.userInfo.club_id : ''
    let res = await axios.get(`/adminapi/activities/filter`, { params: {form,page,size,club_id} })
    tableData.value = res.data.data
    total.value = res.data.total
    if (!total.value) {
...
const actvtDelete = async (actvt) => {
    ...
    if (res.data.ActionType == 'OK') {
        ElNotification({
            title: '删除成功!',
            message: `${actvt.activity_name} 已删除`,
            type: 'success',
            duration: 2000
        })
    }
